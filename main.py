...
async def open_prices(target):
    ...
    msg = target.message if isinstance(target, CallbackQuery) else target
    try:
        await msg.answer_photo(FSInputFile(img_path), caption="Joriy narxlar 👆")
    except Exception:
        pass
    if isinstance(target, CallbackQuery):
        await target.answer()
...
async def choose_category(callback: CallbackQuery):
    ...
    if not products:
        ...
        return
    await safe_edit(callback, f"📦 {cat['name']}\n\nMahsulotni tanlang:", reply_markup=kb.products_kb(products, cat_id))
    await callback.answer()
...
async def choose_product(callback: CallbackQuery, state: FSMContext):
    ...
    await show_purchase_confirmation(callback, product, game_uid=None)
    await callback.answer()
...
async def entered_game_id(message: Message, state: FSMContext):
    ...
    await show_purchase_confirmation(message, product, game_uid=text)


async def show_purchase_confirmation(target, product, game_uid):
    text = (
...
async def entered_topup_amount(message: Message, state: FSMContext):
    await handle_topup_amount(message, state, method="karta")


@dp.callback_query(F.data == "topup_bankomat")
...
PERIOD_MAP = {
    "top_today": lambda: datetime.now().replace(hour=0, minute=0, second=0),
    "top_yesterday": lambda: datetime.now().replace(hour=0, minute=0, second=0) - timedelta(days=1),
...
async def render_top(target, period_key):
    since_fn = PERIOD_MAP.get(period_key, PERIOD_MAP["top_today"])
    since_dt = since_fn()
    since_iso = since_dt.isoformat(timespec="seconds") if since_dt else None

    rows = db.get_top_spenders(since_iso, limit=5)
    medals = ["🥇", "🥈", "🥉", "4.", "5."]
    lines = [
        "🏆 Faollik reytingi (TOP-5)",
        "",
...
@dp.message(F.text == kb.MAIN_MENU_LABELS["settings"])
async def menu_settings_reply(message: Message):
    await open_settings(message)


async def open_settings(target):
    ...
    text = (
        ...
        f"🌐 Til: {lang_names.get(user['language'], '—')}\n\n"
        f"Interfeys tilini tanlang:"
    )
    await send_view(target, text, reply_markup=kb.settings_kb())
...
async def menu_support(callback: CallbackQuery, state: FSMContext):
    await open_support(callback, state)


@dp.message(F.text == kb.MAIN_MENU_LABELS["support"])
async def menu_support_reply(message: Message, state: FSMContext):
    await open_support(message, state)


async def open_support(target, state: FSMContext):
    await state.clear()
    await send_view(target, "☎️ Qo'llab-quvvatlash", reply_markup=kb.support_kb())
...
async def faq_list(callback: CallbackQuery):
    ...
    if not rows:
        text = "Hozircha savol-javoblar qo'shilmagan."
    else:
        lines = ["❓ Tez-tez so'raladigan savollar:\n"]
        for r in rows:
            lines.append(f"▪️ {r['question']}\n{r['answer']}\n")
        text = "\n".join(lines)
    await safe_edit(callback, text, reply_markup=kb.back_kb("menu_support"))
    await callback.answer()


@dp.callback_query(F.data == "support_write")
async def support_write(callback: CallbackQuery, state: FSMContext):
    await state.set_state(Flow.writing_support)
    await safe_edit(callback, "✍️ Xabaringizni yozing, admin tez orada javob beradi:")
    await callback.answer()


@dp.message(Flow.writing_support, lambda m: not (m.text or "").startswith("/"))
async def forward_support_message(message: Message, state: FSMContext):
    photo_id = message.photo[-1].file_id if message.photo else None
    db.add_support_message(message.from_user.id, "user", message.text or message.caption, photo_id)
    await message.answer("✅ Xabaringiz adminga yuborildi.")

    user = db.get_user(message.from_user.id)
    caption = f"✉️ {user['full_name']} (@{user['username']}, ID: {user['user_id']}):\n\n{message.text or message.caption or ''}"
    for admin_row in db.get_all_admins():
        try:
            if photo_id:
                await bot.send_photo(admin_row["user_id"], photo_id, caption=caption)
            else:
                await bot.send_message(admin_row["user_id"], caption)
        except Exception:
            pass
...
async def admin_topup_ok(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        return await callback.answer("Ruxsat yo'q.", show_alert=True)
    code = callback.data.replace("admin_topup_ok_", "")
    topup = db.get_topup_by_code(code)
    if not topup or topup["status"] != "pending":
        return await callback.answer("Allaqachon qayta ishlangan.", show_alert=True)
    db.set_topup_status(code, "approved")
    method_label = TOPUP_METHOD_LABELS.get(topup["method"], topup["method"])
    db.adjust_balance(topup["user_id"], topup["unique_amount"], "topup", note=f"{method_label} — #{code}")
    try:
        await bot.send_message(
            topup["user_id"],
            f"✅ Hisobingiz {topup['unique_amount']:,} so'mga to'ldirildi! ({method_label})".replace(",", " "),
        )
    except Exception:
        pass
...
async def admin_topup_no(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        return await callback.answer("Ruxsat yo'q.", show_alert=True)
    code = callback.data.replace("admin_topup_no_", "")
    db.set_topup_status(code, "rejected")
    topup = db.get_topup_by_code(code)
    try:
        await bot.send_message(topup["user_id"], f"❌ To'ldirish so'rovi #{code} rad etildi.")
    except Exception:
        pass
    await callback.message.edit_caption(caption=(callback.message.caption or "") + "\n\n❌ RAD ETILDI")
    await callback.answer()
...
@dp.callback_query(F.data.startswith("admin_order_ok_"))
async def admin_order_ok(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        return await callback.answer("Ruxsat yo'q.", show_alert=True)
    code = callback.data.replace("admin_order_ok_", "")
    order = db.get_order_by_code(code)
    if not order or order["status"] != "pending":
        return await callback.answer("Allaqachon qayta ishlangan.", show_alert=True)
    db.set_order_status(code, "delivered")
    try:
        await bot.send_message(order["user_id"], f"✅ Buyurtma #{code} bajarildi. Xaridingiz uchun rahmat!")
    except Exception:
        pass
...
@dp.callback_query(F.data.startswith("admin_order_no_"))
async def admin_order_no(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        return await callback.answer("Ruxsat yo'q.", show_alert=True)
    code = callback.data.replace("admin_order_no_", "")
    order = db.get_order_by_code(code)
    if not order or order["status"] != "pending":
        return await callback.answer("Allaqachon qayta ishlangan.", show_alert=True)
    db.set_order_status(code, "rejected")
    db.adjust_balance(order["user_id"], order["price"], "refund", note=f"Rad etildi #{code}")
    try:
        await bot.send_message(order["user_id"], f"❌ Buyurtma #{code} rad etildi, mablag' balansga qaytarildi.")
    except Exception:
        pass
    await safe_edit(callback, callback.message.text + "\n\n❌ RAD ETILDI")
    await callback.answer()
...
# admin.py ichidagi handlerlarni ro'yxatdan o'tkazish
admin_module.register(dp, bot, is_admin, is_super_admin, safe_edit)
